...
import (
	...
	"github.com/SergeyRG/shortener/internal/random"
)
...
func usesKnownPackage(t *testing.T, rootdir string, knownPackages []string) error {
	...
	err := filepath.WalkDir(rootdir, func(path string, d fs.DirEntry, err error) error {
		...
		if spec != nil && spec.Name.String() != "_" {
			...
			return fmt.Errorf("errUsageFound")
		}
	...
	if err == nil {
		return fmt.Errorf("errUsageNotFound")
	}
	// получена специализированная ошибка = мы нашли искомый импорт в файлах
	if errors.Is(err, fmt.Errorf("errUsageFound")) {
		return nil
	}
